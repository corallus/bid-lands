...
def merge_eval(campaign_list):
    ...
    for campaign in campaign_list:
        ...
        for _ in LAPLACE_LIST:
            for mode in MODE_LIST:
                ...
                # evaluation anlp
                i = 0
                ifname = os.path.join(SURVIVAL_MODEL, campaign, mode_name, 'paraTune', params_anlp[mode][campaign], 'evaluation_%s%s.txt' % (campaign, SUFFIX_LIST[mode]))
                fin = open(ifname, 'r')
                lines = fin.readlines()
                for line in lines:
                    if i == 0:
                ...
                # evaluation kld
                i = 0
                ifname = os.path.join(SURVIVAL_MODEL, campaign, mode_name, 'paraTune', params_kld[mode][campaign], 'evaluation_%s%s.txt' % (campaign, SUFFIX_LIST[mode]))
                fin = open(ifname, 'r')
                lines = fin.readlines()
                for line in lines:
                    if i == 0:
    ...
    # print table
    ofname = os.path.join(EVALUATION, 'evaluation.txt')
    fout = open(ofname, 'w')
    # anlp & kld
    fout.write('\t\tANLP\t\t\t\t\tKLD\n')
