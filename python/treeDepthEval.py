...
def getN(info):
    ...
    fout.write(
        'Time cost per sample distribution forecasting: ' + str(float((end_time - start_time)) / len(testset)) + '\n')
    fout.close()
...
def treeDepthEval0(info):
    ...
    fout_evaluation.write(
        "leafSize " + str(info.leafSize) + " treeDepthLimit " + str(info.treeDepth) + " laplace " + str(
            info.laplace) + "\n")
