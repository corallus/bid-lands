...
if __name__ == '__main__':
    ...
    for campaign in CAMPAIGN_LIST:
        ...
        if len(MODE_LIST) == 3:
            ...
            # make length equal
            print "len:", len(q[campaign][NORMAL][laplace]), len(q[campaign][SURVIVAL][laplace]), len(
                q[campaign][FULL][laplace])
            maxLen = max(len(q[campaign][NORMAL][laplace]), len(q[campaign][SURVIVAL][laplace]),
                         len(q[campaign][FULL][laplace]))
            fillLen(q[campaign][NORMAL][laplace], maxLen)
            fillLen(q[campaign][SURVIVAL][laplace], maxLen)
            fillLen(q[campaign][FULL][laplace], maxLen)

            maxLen = max(len(w[campaign][NORMAL][laplace]), len(w[campaign][SURVIVAL][laplace]),
                         len(w[campaign][FULL][laplace]))
